...
class sceneWindow: public ofxFensterListener{
public:
	sceneWindow(){}

	sceneWindow(int w, int h, ofTexture *t=NULL):startX(w),startY(h),scene(t){}

	void draw(){
		ofClear(255,255,255);
		ofFill();

		scene->drawSubsection(0,0,ofGetWidth(),ofGetHeight(),startX,startY,ofGetWidth(),ofGetHeight());
